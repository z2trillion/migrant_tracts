...
class wright_fisher:
	...
	def simulate_mosaic(self,r,ms,Ts,sources):
		...
		while next_position<r:
			...
			contributor,next_recombination	=	contributor.child.recombine(next_position)
	...
	def simulate_tracts(self,r,ms,Ts,sources):
		tile_sources,tile_boundaries=self.simulate_mosaic(r,ms,Ts,sources)
		switch_points			=	[s!=s_next for s,s_next in zip(tile_sources[:-1],tile_sources[1:])]
		switch_points.append(True)
		
		tract_sources			=	[tile_sources[i] for i in range(len(tile_sources)) if switch_points[i]]
		tract_boundaries	=	[tile_boundaries[i] for i in range(len(tile_sources)) if switch_points[i]]
...
class person:
	...
	def get_copying(self):
		if self.copying is None:
			...
			try:
				...
				self.wf.population[(self.generation+1,parent_ID)] = person(child=self)
				self.copying=self.wf.population[(self.generation+1,parent_ID)]
		return self.copying
	...
	def get_ancestor(self,position):
		if self.from_source_population:
			...
			if np.random.uniform()<.5-.5*np.exp(2*(self.last_seen_at-position)):
				self.swap_copying()
...
if __name__=='__main__':
	...
	parser.add_argument('-N',help='effective number of diploid people in population')
	...
	for j in range(quantidade):
		...
		for i in range(len(sources)):
			print ('%s\t%f\t%s\t%s'%(sources[i],tract_lengths[i],j,cromossomo))
